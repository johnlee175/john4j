...
import com.johnsoft.library.swing.component.JohnSpinner;
import com.johnsoft.library.swing.component.datechooser.JohnDateField.PatternType;
import com.johnsoft.library.util.common.JohnStringUtil;
import com.johnsoft.library.util.gui.JohnSwingUtilities;


public class JohnDatePicker extends JPanel
{
	...
	private AWTEventListener awtel=new AWTEventListener()
	{
		@Override
		public void eventDispatched(AWTEvent e)
		{
			if(e.getID()==MouseEvent.MOUSE_PRESSED)
			{
				...
				Component c=SwingUtilities.getRoot((Component)obj);
				if(obj!=dateButton&&obj!=dateField&&obj!=dialog&&!c.getClass().getName().equals("javax.swing.Popup$HeavyWeightWindow"))
					dialog.setVisible(false);
			}
		}
	};
	
	private ComponentAdapter capter = new ComponentAdapter()
	{
		@Override
	...
	private WindowAdapter wapter=new WindowAdapter()
	{
		@Override
	...
	// 获取实例与构造方法开始
	public JohnDatePicker()
	{
		this(new Date());
	}

	public JohnDatePicker(Date date)
	{
		this(date, false);
	}

	public JohnDatePicker(boolean isChina)
	{
		this(new Date(), isChina);
	}

	public JohnDatePicker(String format)
	{
		this(new Date(), format);
	}

	public JohnDatePicker(Date date, boolean isChina)
	{
		this(date, "yyyy-MM-dd HH:mm:ss", isChina);
	}

	public JohnDatePicker(String format, boolean isChina)
	{
		this(new Date(), format, isChina);
	}

	public JohnDatePicker(Date date, String format)
	{
		this(date, format, true);
	}

	public JohnDatePicker(Date date, String format, boolean isChina)
	{
...
//		mainPanel.add(footPane, BorderLayout.SOUTH);
	...
	/**
	 * 将日历控件注册给文本框
	 * @param key			如果传2,日历面板上从当天起仅两日可选,否则为从当天起全月可选
	 * @param dateField		应是JTextComponent的子类
	 * @param win			请传null
	 * @param dateButton	请传dateField
	 */
			...
			final JComponent dateButton)
	{
		...
		((JTextComponent)dateField).setDocument(new MaskManMadePlainDoc());
	...
	protected void hidePanel()
	{
		...
		JohnSwingUtilities.removeProgramInputEventListener(awtel);
		Window win=SwingUtilities.windowForComponent(dateField);
		win.removeComponentListener(capter);
		win.removeWindowListener(wapter);
	}
	...
	protected void showPanel()
	{
		if(isEnabled())
		{
			...
			setDialogShow();
			fireChooserShow();
			JohnSwingUtilities.addProgramInputEventListener(awtel);
			Window win=SwingUtilities.windowForComponent(dateField);
			win.addComponentListener(capter);
			win.addWindowListener(wapter);
		}
	}
			...
			MouseListener, MouseMotionListener
	{
		...
		public MyLabel(int year, int month, int day, int index)
		{
			...
			this.day = day;
			this.month = month;
//			Integer yearNew=select.get(Calendar.YEAR);    
//			Integer monthNew=select.get(Calendar.MONTH);    
//			Integer dayNew=select.get(Calendar.DATE);   
			Integer date;
			String dayNew;
			String monthNew;
			if(day<10)
			...
			date=Integer.parseInt(year+""+monthNew+""+dayNew);
			
			this.addMouseListener(this);
			this.addMouseMotionListener(this);
			this.setFont(dateFont);
//			if (month == select.get(Calendar.MONTH))
			if ((date >= getCurrentDate()) && (month == select.get(Calendar.MONTH)))
			{
				setOnDateCursor();
...
//			if ((month == select.get(Calendar.MONTH)) && (day >= select.get(Calendar.DAY_OF_MONTH)))
			{
				setOnDateCursor();
				this.addMouseListener(new MouseAdapter()
				{
...
//			}
	...
	protected class MaskManMadePlainDoc extends PlainDocument
	{
		private static final long serialVersionUID = 1L;
		@Override
	...
	{    
		...
		Integer date=Integer.parseInt((year+""+monthNew+""+dayNew));
