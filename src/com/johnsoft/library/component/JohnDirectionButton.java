...
public class JohnDirectionButton extends JButton
{
	private static final long serialVersionUID = 1L;

	public enum JohnDirect
	{
		JD_LEFT, JD_RIGHT
	};

	protected JohnDirectionButton.JohnDirect direction;
	protected Map<?, ?> desktopHints;
	protected float morphing = 0.0f;

	public JohnDirectionButton(String text, Icon icon, JohnDirect direction)
	{
		super(text,icon);
	...
	protected void paintComponent(Graphics g)
	{
		...
		if (!getModel().isArmed())
		{
			colors = new Color[] { new Color(0x63a5f7), new Color(0x3799f4),
					new Color(0x2d7eeb), new Color(0x30a5f9) };
		} else
		{
			colors = new Color[] { new Color(0x63a5f7).darker(),
					new Color(0x3799f4).darker(), new Color(0x2d7eeb).darker(),
					new Color(0x30a5f9).darker() };
		}

		p = new LinearGradientPaint(0.0f, 0.0f, 0.0f, getHeight(), new float[] {
				0.0f, 0.5f, 0.501f, 1.0f }, colors);

		g2.setPaint(p);

		Morphing2D morph = createMorph();
		morph.setMorphing(getMorphing());
		if (direction == JohnDirect.JD_RIGHT)
		{
	...
	public static double interp(double v0, double v1, double t)
	{
		return (v0 + ((v1 - v0) * t));
	}
	
	public static double[] mergeTvals(double tvals0[], double tvals1[])
	{
		int i0 = 0;
		int i1 = 0;
		int numtvals = 0;
	...
	{
		protected double morph;
		protected Geometry startGeometry;
		protected Geometry endGeometry;

		/**
		 * <p>
		 * Creates a new morphing shape. A morphing shape can be used to turn
		 * one shape into another one. The transformation can be controlled by
		 * the morph property.
		 * </p>
		 * 
		 * @param startShape
		 *            the shape to morph from
		 * @param endShape
		 *            the shape to morph to
		 * 
		 * @throws IllegalPathStateException
		 *             if the shapes do not have the same winding rule
		 * @see #getMorphing()
		 ...
		 * the transformation from the start shape to the end shape. A value of
		 * 0.0 is the start shap. A value of 1.0 is the end shape. A value of
		 * 0.5 is a new shape, morphed half way from the start shape to the end
		 * shape.
		 * </p>
		 * <p>
		...
		public Rectangle2D getBounds2D()
		{
			...
			xmin = xmax = interp(startGeometry.getCoord(0),
					endGeometry.getCoord(0), morph);
			ymin = ymax = interp(startGeometry.getCoord(1),
					endGeometry.getCoord(1), morph);
			for (int i = 2; i < n; i += 2)
			{
	...
	protected static class Geometry
	{
		...
		public Geometry(Shape s)
		{
			...
			if ((numCoords < 8) || curx != bezierCoords[0]
					|| cury != bezierCoords[1])
			{
				newx = bezierCoords[0];
				newy = bezierCoords[1];
			...
			if (minPt > 0)
			{
				...
				System.arraycopy(bezierCoords, minPt, newCoords, 0, numCoords
						- minPt);
				...
				System.arraycopy(bezierCoords, 2, newCoords, numCoords - minPt,
						minPt);
				bezierCoords = newCoords;
			}
			/*
			 * Clockwise enforcement: - This technique is based on the formula
			 * for calculating the area of a Polygon. The standard formula is:
			 * Area(Poly) = 1/2 * sum(x[i]*y[i+1] - x[i+1]y[i]) - The returned
			 * area is negative if the polygon is "mostly clockwise" and
			 * positive if the polygon is "mostly counter-clockwise". - One
			 * failure mode of the Area calculation is if the Polygon is
			 * self-intersecting. This is due to the fact that the areas on each
			 * side of the self-intersection are bounded by segments which have
			 * opposite winding direction. Thus, those areas will have opposite
			 * signs on the acccumulation of their area summations and end up
			 * canceling each other out partially. - This failure mode of the
			 * algorithm in determining the exact magnitude of the area is not
			 * actually a big problem for our needs here since we are only using
			 * the sign of the resulting area to figure out the overall winding
			 * direction of the path. If self-intersections cause different
			 * parts of the path to disagree as to the local winding direction,
			 * that is no matter as we just wait for the final answer to tell us
			 * which winding direction had greater representation. If the final
			 * result is zero then the path was equal parts clockwise and
			 * counter-clockwise and we do not care about which way we order it
			 * as either way will require half of the path to unwind and re-wind
			 * itself.
			 */
			double area = 0;
			...
			{
				/*
				 * The area is negative so the shape was clockwise in a
				 * Euclidean sense. But, our screen coordinate systems have the
				 * origin in the upper left so they are flipped. Thus, this path
				 * "looks" ccw on the screen so we are flipping it to "look"
				 * clockwise. Note that the first and last points are the same
				 * so we do not need to swap them. (Not that it matters whether
				 * the paths end up cw or ccw in the end as long as all of them
				 * are the same, but above we called this section "Clockwise
		...
		public double[] getTvals()
		{
			...
			while (ci < numCoords)
			{
				...
				/*
				 * If the result is zero length then we will have problems below
				 * trying to do the math and bookkeeping to split the segment or
				 * pair it against the segments in the other shape. Since these
				 * lengths are just estimates to map the segments of the two
				 * shapes onto corresponding segments of
				 * "approximately the same length", we will simply fudge the
				 * length of this segment to be at least a minimum value and it
				 * will simply grow from zero or near zero length to a
				 * non-trivial size as it morphs.
				 */
				if (len < MIN_LEN)
				{
					len = MIN_LEN;
